...
def verify(directory):
    ...
    if (record.get('status')!='ok' or record.get('schema_version')!=2
            ...
            or set(tables)!=set(SCHEMA['tables'])|{'orders_nested'}):
...
def ddl(engine):
    ...
    return f'-- Canonical schema; nested non-null and key invariants are validated by datagen.\nCREATE TABLE orders_nested (\n    {header},\n    lineitems {nested} NOT NULL\n)'+(' USING PARQUET' if engine=='spark' else '')+';\n'
...
def generate(inputs,out,fmt,label,memory_limit='4GB',key_span=250000):
    ...
    nested=out/'orders_nested.parquet';nested.mkdir()
    ...
    statements.append(f"CREATE VIEW orders_nested AS SELECT * FROM read_parquet({literal(nested/'*.parquet')});")
    ...
    statements.append(f'CREATE VIEW leaves AS SELECT {leaves} FROM orders_nested, UNNEST(lineitems) AS u(x);')
    for original,derived in [('SELECT * FROM orders',f'SELECT {headers} FROM orders_nested'),(f'SELECT {flat} FROM lineitem',f'SELECT {flat} FROM leaves')]:
    ...
    specs={**SCHEMA['tables'],'orders_nested':{'primary_key':['o_orderkey']}}
    for name,spec in SCHEMA['tables'].items():
