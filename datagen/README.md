...
devenv --profile duckdb shell -- python3 datagen/generate.py \
  ...
  --out workspaces/shared/sf1 \
  --source-label 'TPC-H dbgen <revision>; SF1'

devenv --profile duckdb shell -- python3 datagen/generate.py \
  --verify workspaces/shared/sf1
```
...
devenv --profile duckdb shell -- python3 datagen/generate.py \
  ...
  --out workspaces/shared/converted --source-label 'source dataset identity'
```
...
then flushes each range to `orders_nested.parquet/part-*.parquet`. Flat tables
...
devenv --profile spark shell -- "$HOME/ndc-spark41/spark-4.1.3-bin-hadoop3/bin/spark-submit" \
  ...
  --input /path/to/flat-parquet --input-format parquet \
  --out workspaces/shared/spark-prepared --source-label 'source dataset identity'
```
