...
def create(campaign,source,destination):
    ...
    for cell in campaign.glob('spark_*.json'):
        ...
        if record.get('fmt') in ('delta','iceberg'):
            marker=json.loads((workspace/f'format-{record["fmt"]}.json').read_text())
    ...
    qualification=campaign.parent/'qualification.json'
    if qualification.is_file():
        for path in (qualification,*sorted(campaign.parent.glob('*.log'))):
            if path.is_file() and not path.is_symlink():files['qualification/'+path.name]=path.read_bytes()
