...
with tempfile.TemporaryDirectory() as tmp:
    ...
    try:
        ...
        spark.read.parquet(str(out/'orders_nested.parquet')).write.insertInto('orders_nested')
        nested=spark.table('orders_nested')
        ...
        leaves=spark.sql('SELECT o_orderkey AS l_orderkey,li.* FROM orders_nested LATERAL VIEW explode(lineitems) u AS li')
        ...
        spark.createDataFrame([(1,None),(2,[]),(3,[None]),(4,[(1,),(1,)])],
                              'o_orderkey long,lineitems array<struct<l_linenumber:int>>').createOrReplaceTempView('orders_nested')
